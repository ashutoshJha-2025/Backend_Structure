...
const Login = () => {
    ...
    const sendData = async () => {
        try {
            const result = await axios.post('https://backend-0pln.onrender.com/api/user/authentication/login', formData, { withCredentials: true })
    ...
    return (
        <div className="w-full h-full flex items-center justify-center px-4">
            <div className="w-100  bg-white rounded-3xl shadow-2xl px-8 py-4 relative top-10">
                ...
                <form onSubmit={(e) => e.preventDefault()} className="mt-6 space-y-4">
                    ...
                    <div className="flex items-center gap-3 ">
                        <p>Don't have an account? <Link to='/auth/register' className='text-sky-600 font-semibold '>Sign up</Link></p>
                    </div>
                </form>
            </div>
        </div>
